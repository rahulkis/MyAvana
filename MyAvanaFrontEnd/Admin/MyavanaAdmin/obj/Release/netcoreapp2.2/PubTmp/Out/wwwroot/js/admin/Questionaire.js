...
function onRender(data, type, row, meta) {
    ...
    tr += '<a href="javascript:void(0);" onclick="redirectToView(\'' + row.UserId + '\')" Title="Move to questionnaire"><i class="fa fa-user viewuser" aria-hidden="true"></i></a>';
    return tr;
}
function redirectToView(userId) {
    window.location.href = '/Questionnaire/QuestionnaireList?userId=' + userId;
}
function moveToHairProfile(userEmail, userName, questionnaireCompleted,UserId) {
    if (questionnaireCompleted.toLowerCase() === "yes") {
       
        $('#AutoGenerateHHCPModal').modal('show');
        // Attach an event handler to the Continue button
